(function () {
    ...
    return fetch(url, {
      ...
      if (!r.ok) {
        return r.json().then(function (err) {
          console.error('[vivid-cart] API ' + r.status + ' from ' + url + ': ' + JSON.stringify(err));
  ...
  function fail(context, row, err) {
    console.error('[vivid-cart] ' + context + ' failed:', err);
    busy = false;
    stopLoading(row);
    ...
    btn.addEventListener('click', function () {
      ...
      getCart()
        .then(function (cart) {
          ...
          console.log('[vivid-cart] changing qty for key:', item.key, '→', qty);
